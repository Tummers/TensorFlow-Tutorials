...
def load_training():
    batch_size = 32
    seed = 42
    raw_train_ds = tf.keras.preprocessing.text_dataset_from_directory("StackOverflow/train",
                                                                      batch_size=batch_size,
                                                                      validation_split=0.2,
                                                                      subset="training",
                                                                      seed=seed)
    
    raw_validation_ds = tf.keras.preprocessing.text_dataset_from_directory("StackOverflow/train",
                                                                      batch_size=batch_size,
                                                                      validation_split=0.2,
                                                                      subset="validation",
                                                                      seed=seed)
    
    raw_test_ds = tf.keras.preprocessing.text_dataset_from_directory("StackOverflow/test",
                                                                    batch_size=batch_size)
